...
function Sale({ storeId }) {
  ...
  const monnaie = !isNaN(montantRecuNum) ? montantRecuNum - total : null
  const montantInsuffisant = monnaie !== null && monnaie < 0

  const suggestionsMontant = [500, 1000, 2000]
    ...
    <div className="min-h-screen pb-24">
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-16 left-0 right-0 bg-white/[0.04] backdrop-blur-xl border-t border-white/10 rounded-t-3xl shadow-elevated-lg p-4 z-30">
          ...
          <button
            onClick={() => { setMontantRecu(''); setConfirming(true) }}
      ...
      {confirming && (
        <div className="fixed inset-0 bg-black/60 flex items-end z-50">
          <div className="surface rounded-t-3xl w-full p-6">
            ...
            {paymentMethod === 'cash' && (
              <div className="mb-6">
                <p className="text-sm text-muted mb-2">{t('cashReceived')}</p>
                <input
                  type="number"
                  step="0.01"
                  placeholder={`${total.toFixed(2)} ${t('currency')}`}
                  value={montantRecu}
                  onChange={(e) => setMontantRecu(e.target.value)}
                  className="input-field mb-2.5"
                />
                <div className="flex gap-2 mb-3">
                  {suggestionsMontant.map((v) => (
                    <button
                      key={v}
                ...
                {monnaie !== null && (
                  montantInsuffisant ? (
                    <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-rose-400/10 border border-rose-400/20">
                      <span className="text-sm text-danger font-medium">{t('insufficientAmount')}</span>
                    </div>
                  ) : (
                    <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-emerald-400/10 border border-emerald-400/20">
                      <span className="text-sm text-muted">{t('changeDue')}</span>
                      <span className="text-lg font-display font-semibold text-success">{monnaie.toFixed(2)} {t('currency')}</span>
                    </div>
                  )
                )}
              </div>
            ...
            <button
              ...
              disabled={processing || montantInsuffisant}
