        ...
        <div className="space-y-2">
          {movements.map((m) => (
            <div key={m.id} className="glass-card p-4 flex items-center gap-3">
              {m.type === 'in' ? (
                <ArrowDownCircle size={22} className="text-success shrink-0" />
              ) : (
                <ArrowUpCircle size={22} className="text-danger shrink-0" />
              )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end z-50">
          <div className="surface rounded-t-3xl w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="heading text-lg">{t('stockMovementModalTitle')}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl font-medium border transition-colors ${form.type === 'in' ? 'bg-success/20 text-success border-success/40' : 'bg-white/5 text-violet-300/70 border-white/10'}`}
                >
                  {t('movementIn')}
                </button>
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl font-medium border transition-colors ${form.type === 'out' ? 'bg-danger/20 text-danger border-danger/40' : 'bg-white/5 text-violet-300/70 border-white/10'}`}
                >
                  {t('movementOut')}
                </button>
              </div>
