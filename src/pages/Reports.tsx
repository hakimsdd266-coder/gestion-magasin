          ...
          <>
            <div className="glass-card p-5 mb-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm text-muted">{t('profitLabel')}</span>
                {isProfit ? (
              ...
              <p className={`text-3xl font-display font-semibold ${isProfit ? 'text-success' : 'text-danger'}`}>
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              <div className="glass-card p-3">
                <p className="text-xs text-muted mb-1">{t('revenueLabel')}</p>
                <p className="font-semibold text-violet-50 text-sm">{stats.revenue.toFixed(2)}</p>
              </div>
              <div className="glass-card p-3">
                <p className="text-xs text-muted mb-1">{t('costLabel')}</p>
                <p className="font-semibold text-violet-50 text-sm">{stats.cost.toFixed(2)}</p>
              </div>
              <div className="glass-card p-3">
                <p className="text-xs text-muted mb-1">{t('expensesLabel')}</p>
            ...
            <div className="space-y-2">
              {expenses.map((exp) => (
                <div key={exp.id} className="glass-card p-4 flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="badge-warn">{t(`expenseCategory${exp.category.charAt(0).toUpperCase() + exp.category.slice(1)}`)}</span>
                  ...
                  <div className="flex items-center gap-3 ml-2">
                    ...
                    <button onClick={() => supprimer(exp.id)} className="text-danger hover:text-rose-300 transition-colors">
                      <Trash2 size={18} />
                    </button>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-end z-50">
          <div className="surface rounded-t-3xl w-full p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="heading text-lg">{t('addExpense')}</h2>
